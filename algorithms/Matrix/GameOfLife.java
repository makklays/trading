 ...
 *
 * Каждая ячейка взаимодействует со своими восемью соседями (по горизонтали, вертикали и диагонали) по следующим четырем правилам (взятым из игры «Жизнь» Джона Конвея):
 * - Любая живая клетка, у которой меньше двух живых соседей, погибает (как от малонаселенности).
 * - Любая живая клетка, у которой два или три живых соседа, выживает и переходит в следующее поколение.
 * - Любая живая клетка, у которой больше трех живых соседей, погибает (как от перенаселенности).
 * - Любая мертвая клетка, у которой ровно три живых соседа, оживает (как от размножения). 
 *
 * Example 1:
...
class Solution {
    ...
    private int countLiveNeighbors(int[][] board, int r, int c, int m, int n) {
        ...
        // чтобы не выйти за границы матрицы 3x3 и не получить ArrayIndexOutOfBoundsException 
